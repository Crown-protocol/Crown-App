  ...
  const groups: { title: string; rows: Row[] }[] = [
    {
      ...
      rows: [
        ...
        { name: "Two-outcome factory (game escrows)", detail: short(FACTORY_TWO_OUTCOME.toBase58()), state: "on", note: "NEXT_PUBLIC_FACTORY_TWO_OUTCOME — the whole mainnet form set" },
    ...
    },
    {
      ...
      rows: [
        ...
        { name: "crown-indexer (reputation book)", detail: CHEER_INDEX_PRINCIPAL || "NEXT_PUBLIC_CHEER_INDEX_PRINCIPAL", state: isIndexConfigured() ? "on" : "off", note: "until then the DB mirror serves /api/reputation" },
        { name: "crown-relay (pays for ingests and signatures)", detail: RELAY_PRINCIPAL || "NEXT_PUBLIC_RELAY_PRINCIPAL", state: RELAY_PRINCIPAL ? "on" : "off", note: "plus the server-side RELAY_IDENTITY_JSON — the allowlisted key" },
